...
EXCEL_PATH = os.path.join(SCRIPT_DIR, "042026 waldenstrom_staging_consolidated.xlsx")
...
def load_existing_review_sheet(excel_path: str, observer: str) -> tuple[dict, str | None]:
    """Check for existing Review sheets and load the most recent one for this observer."""
    staging = {}
    sheet_name = None
    try:
...
def save_to_review_sheet(excel_path: str, image_list: list[dict],
                         staging: dict, observer: str, sheet_name: str):
    """Save current staging to a review sheet in the Excel file."""
    try:
    ...
    headers = ["ap_file_name", "patient_token", "laterality",
               "Joonoh", "Ahan", "Kevin", obs_col, "Disagreement"]
    ws.append(headers)
...
class StagingApp(tk.Tk):
    def __init__(self, active_observer: str):
        ...
        # Load data
        print("Loading consolidated Excel...")
        df = load_consolidated_excel(EXCEL_PATH)
        ...
        if session and session.get("observer") == self.active_observer and session.get("sheet_name"):
            existing, found_sheet = load_existing_review_sheet(EXCEL_PATH, self.active_observer)
    ...
    def _load_current(self):
        ...
        for obs in OBSERVERS:
            ...
            # Highlight disagreement: compare to majority
            stages = [img_data.get(o) for o in OBSERVERS if img_data.get(o)]
            if len(stages) >= 2:
    ...
    def _update_timeline(self):
        ...
        for img in series:
            ...
            # Mini observer stages: J A K
            for obs_short, obs_full in [("J", "Joonoh"), ("A", "Ahan"), ("K", "Kevin")]:
    ...
    def _apply_stage(self, stage: str):
        ...
        def _bg_save():
            with _save_lock:
                save_to_review_sheet(EXCEL_PATH, self.image_list,
